...
SOLUTION_FILE = 'Nancy.Bootstrappers.DryIoc.sln'
...
assemblyinfo :version => [:clean] do |asm|
  NANCY_VERSION = get_assembly_version SHARED_ASSEMBLY_INFO
    ...
    asm.product_name = "Nancy.Bootstrappers.DryIoc"
    asm.title = "Nancy.Bootstrappers.DryIoc"
    asm.description = "A DryIoc Bootstrapper for the Nancy web framework"
    asm.copyright = "Copyright (C) Maksim Volkau and contributors"
...
xunit :xunit => [:compile] do |xunit|
    tests = FileList["test/**/#{CONFIGURATION}/Nancy.Bootstrappers.DryIoc.Tests.dll"].exclude(/obj\//)
...
zip :package => [:publish] do |zip|
    ...
    zip.output_file = "Nancy.Bootstrappers.DryIoc-Latest.zip"
...
task :nuget_package => [:publish] do
    ...
    nuspecs = FileList["src/nancy.bootstrappers.DryIoc/*.nuspec"]
    root = File.dirname(__FILE__)

    # Copy all project *.nuspec to nuget build folder before editing
    FileUtils.cp_r nuspecs, "#{OUTPUT}/nuget"
    nuspecs = FileList["#{OUTPUT}/nuget/nancy.bootstrappers.DryIoc.nuspec"]
    ...
    nuspecs.each do |nuspec|
        update_xml nuspec do |xml|
            ...
            xml.root.elements["metadata/authors"].text = "Maksim Volkau and contributors"
            ...
            xml.root.elements["metadata/licenseUrl"].text = "https://github.com/NancyFx/Nancy.Bootstrappers.DryIoc/blob/master/license.txt"
